...
void compressFileHC(const char* inputFileName, const char* outputFileName)
{
    ...
    LZ4_streamHC_t* const lz4Stream = LZ4_createStreamHC();
    ...
    while(true)
    {
        ...
        compressedBytes = LZ4_compress_HC_continue(lz4Stream , inputBufferPtr, compressedBuffer, inputFile.gcount(), destinationBufferSize);
    ...
    std::cout << "HC Compression is successful. Output written to: " << outputFileName << std::endl;
